...
def connectdb():
    ...
    cursor = conn.cursor()
    cursor.execute(
        """ CREATE TABLE IF NOT EXISTS usuarios(email TEXT NOT NULL, password TEXT NOT NULL , nome TEXT NOT null, sessionID TEXT)"""
    )
    conn.commit()
    return conn, cursor
...
def loginverify(usuario, senha):
    ...
    dados = cursor.execute(
        """ SELECT password from usuarios where email = (?)""", (usuario,)
    ).fetchall()
...
def register(nome, email, passw):
    ...
    if dados == []:
        cursor.execute(
            """ INSERT INTO usuarios(email,password,nome, sessionID) VALUES(?,?,?,?) """,
            (
                email,
                passw,
                nome,
...
class cookies:
    def verify(sessionID):
        conn, cursor = connectdb()
        dados = cursor.execute(
            """ SELECT * FROM usuarios WHERE sessionID = (?) """, (sessionID,)
        ).fetchall()
    ...
    def create(email, senha):
        ...
        cursor.execute(
            """ UPDATE usuarios SET sessionID = (?) WHERE email = (?) AND password = (?) """,
            (
                cookie,
                email,
                senha,
    ...
    def delete(sessionID):
        ...
        if not dados == [] and sessionID != "None":
            cursor.execute(
                """ UPDATE usuarios SET sessionID = (?) WHERE sessionID = (?)""",
                (
                    "None",
                    sessionID,
                ),
